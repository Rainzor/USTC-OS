...
int split_string(char* string, char *sep, char** string_clips) {
    ...
    do {
        ...
        tail = head + strlen(string_clips[clip_num]) - 1;//head and tail of one of strings 
        while(*head == ' ' && head != tail)//remove 'space'
            head ++;
        ...
        *(tail + 1) = '\0';
        ...
        clip_num ++;
    }while(string_clips[clip_num]=strtok(NULL, sep));//cut the string until return NULL
    return clip_num;
}
...
int process_redirect(int argc, char** argv, int *fd) {
    ...
    while(i < argc) {
        ...
        if(strcmp(argv[i], ">") == 0) {
            ...
            argv[j++] = argv[i++];//remain the true command
        }
    }
    argv[j] = NULL;
    return j;   // 新的argc
...
int main() {
    ...
    char *commands[128];//At most 128 commands in one line
    char *multi_cmds[32];//At most 32 commands 
    int cmd_count,muli_count;
    while (1) {
        ...
        muli_count=split_string(cmdline,";",multi_cmds);
        for(int item=0;item<muli_count;item++){        
            /* 由管道操作符'|'分割的命令行各个部分，每个部分是一条命令 */
            /* 拆解命令行 */
            cmd_count = split_string(multi_cmds[item], "|", commands);
            if(cmd_count == 0) {
                continue;
                ...
                if(pid == 0) {  
                    ...
                    dup2(pipefd[WRITE_END], STDOUT_FILENO);//without dup2 ,the child process will output the massage on the STDOUT,
                    close(pipefd[WRITE_END]);                                      //but using dup2, the massage will output into pipe
                    /* 
                ...
                // 而是直接创建下一个子进程
                //pipe can be used for two siblings
                // 子进程2
                pid = fork();
                if(pid == 0) {  
                ...
                while (wait(NULL) > 0);//when parents don't have any child process ,"wait" return -1
            } else {    // 选做：三个以上的命令
