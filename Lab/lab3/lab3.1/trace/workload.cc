...
void* workload_run(void *workload){
    ...
    for(int loop=0; loop<LOOP_NUM; loop++){
        ...
        std::cout<<"insert time :"<<(sec2-sec1)*1000 + (usec2-usec1)/1000 << "ms" << std::endl;        
        ...
        gettimeofday(&cur_time, NULL);
        sec2=cur_time.tv_sec,usec2=cur_time.tv_usec;
        std::cout<<"total time"<< loop <<" : "<<(sec2-sec1)*1000 + (usec2-usec1)/1000 << "ms\n" << std::endl;        
    }
}
