...
void mem_init(void)
{
    /*
        ...
        此处增长堆空间大小为 MAX_HEAP
    */
    mem_start_brk = (char *)sbrk(MAX_HEAP);//5MB = 5*(1<<20) 以字节为单位，作为起始地址
    mem_brk = (char *)mem_start_brk;
    mem_max_addr=(char *) (mem_start_brk+MAX_HEAP);//char地址是按字节为单位 
 ...
 */
void *mem_sbrk(int incr) //incr以一个字节为单位输入
{
    if(incr == 0)
        return (void *) mem_brk;
    if(incr < 0)
    ...
    */
    char* next_mem_brk = mem_brk + incr;//以字节增长

    while(next_mem_brk >= mem_max_addr){
        if(sbrk(MAX_HEAP) == (void *) -1)
            return (void *) -1;
        mem_max_addr += MAX_HEAP; 
       // printf("mem increased\n");

    }
    mem_brk = next_mem_brk; 
    return (void *)old_brk;
}
