...
// 定义要输出的文件
#define OUTPUT_FILE "/home/rainzor/oslab/lab3/lab3.2/expr_result.txt"
...
static my_page_referenced mpage_referenced = (my_page_referenced)0xffffffff96ab0d10;
// sudo cat /proc/kallsyms | grep follow_page
static my_follow_page mfollow_page = (my_follow_page)0xffffffff96a94450;
...
static void scan_vma(void)
{
    ...
    if (mm)
    {
        ...
        do{
            vma_count ++;
            //printk("vma:%d, 0x%lx",vma_count, vma->vm_start);
        }while((vma = vma->vm_next) != NULL);
        my_task_info.vma_cnt=vma_count;
        mmput(mm);
    }
}
...
static unsigned long virt2phys(struct mm_struct *mm, unsigned long vaddr)
{
    ...
    page = pte_page(*pte);
    //page_addr = pte_val(*pte) & PAGE_MASK;    //高48位
    //page_offset = vaddr & ~PAGE_MASK;           //低12位
    //phy_addr = page_addr | page_offset;          //拼接address

    //phy_addr = (pte_val(*pte) & PAGE_MASK) |(va & ~PAGE_MASK);     
    //return phy_addr; 
    if (page)
        return pte_pfn(*pte);
    else{
...
static void traverse_page_table(struct task_struct *task)
{
    ...
    if (mm)
    {
        ...
        struct page * page;
        // TODO:遍历VMA，并以PAGE_SIZE为力度逐个遍历VMA中的虚拟地址，然后进行页表遍历
        do
        {
            for (virt_addr = vma->vm_start; virt_addr < (vma->vm_end) ; virt_addr += PAGE_SIZE){
                page = mfollow_page(vma, virt_addr, FOLL_GET);
                if(!IS_ERR_OR_NULL(page)){
                    phys_addr=virt2phys(task->mm, virt_addr);
                    if(phys_addr>0)
                        record_two_data(virt_addr, phys_addr);
                    else
                        pr_err("func: %s page is NULL\n", __func__);
                }
            }
            count++;
            //printk("vma:%d, 0x%lx",count, vma->vm_start);
            flush_buf(1);
        } while ((vma = vma->vm_next) != NULL);
...
{
    struct mm_struct *mm;
    unsigned long addr_end,addr_start,virt_addr,addr;
    ...
    switch (ktest_func){
        case 4:
            addr_start= mm->start_data;
            addr_end = mm->end_data;
            break;
        case 5:
            addr_start = mm->start_code;
            addr_end = mm->end_code;
            break;
    }
    for(virt_addr = addr_start; virt_addr < addr_end; virt_addr += PAGE_SIZE){
        vma_tmp = find_vma(mm, virt_addr);
        page = mfollow_page(vma_tmp, virt_addr, FOLL_GET);
    ...
    //printk("start: 0x%lx, end: 0x%lx, vma_start:0x%lx", addr_start,addr_end,vma_tmp->vm_start);
    mmput(mm);
}
