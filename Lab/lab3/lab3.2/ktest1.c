...
static my_page_referenced mpage_referenced = (my_page_referenced)0xffffffffaf4b0d10;
// sudo cat /proc/kallsyms | grep follow_page
static my_follow_page mfollow_page = (my_follow_page)0xffffffffaf494450;
...
static void scan_vma(void)
{
    ...
    if (mm)
    {
        ...
        struct vm_area_struct* vma_temp=NULL;
        int vma_count=0;
        vma_temp = mm->mmap;
        do{
            vma_count ++;
        }while((vma_temp = vma_temp->vm_next) != NULL);
...
static void print_mm_active_info(void)
{
    ...
    if (mm){
        ...
        do
        {
            for (virt_addr = vma_temp->vm_start; virt_addr < (vma_temp->vm_end) ; virt_addr += PAGE_SIZE){
                struct page *page = (struct page *)mfollow_page(vma_temp, virt_addr, FOLL_GET);
                if(page!=NULL && (mpage_referenced(page, 0, (struct mem_cgroup *)(page->memcg_data), &(vma_temp->vm_flags)))){
