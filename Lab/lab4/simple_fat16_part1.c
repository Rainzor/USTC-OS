...
/**
 * @brief 读取扇区号为secnum的扇区，将数据存储到buffer中(只要有了扇区号，就能读内容)
 * 
 * @param fd      镜像文件指针
...
/**
 * @brief 将buffer中的数据写入到扇区号为secnum的扇区中(只要有了扇区号，就能写内容)
 *
 * @param fd      镜像文件指针
...
/**
 * @brief 返回簇号为ClusterN对应的FAT表项(即下一个簇号)
 * 
...
WORD fat_entry_by_cluster(FAT16 *fat16_ins, WORD ClusterN)
{
  ...
  WORD FATOffset = ClusterN * 2;//ClusterN长度为两字节，对于sector_buffer仍然按字节为单位存储，所以需要将ClusterN*2，计算出在buffer中正确的偏移量
  /* FatSecNum is the sector number of the FAT sector that contains the entry
   * for cluster N in the first FAT */
  WORD FatSecNum = fat16_ins->Bpb.BPB_RsvdSecCnt + (FATOffset / fat16_ins->Bpb.BPB_BytsPerSec);//因为是按扇区来读Buffer的，所以应当先找到到簇以扇区为单位的偏移量
 ...
 * 
 * @param fat16_ins   文件系统元数据指针
 * @param Root        输出参数，该文件或目录在目录表中对应的目录项内容
 * @param path        输入要查找的路径
...
int find_root(FAT16 *fat16_ins, DIR_ENTRY *Root, const char *path, off_t *offset_dir)
{
  ...
  {
    memcpy(Root, &buffer[((i - 1) * BYTES_PER_DIR) % BYTES_PER_SECTOR], BYTES_PER_DIR);//取buffer中一个目录条目大小（32B）的内容

    /* If the directory entry is free, all the next directory entries are also
     * free. So this file/directory could not be found */
    if (Root->DIR_Name[0] == 0x00)//以1字节为单位
    {
      return 1;
    }
    ...
    {
      *offset_dir = fat16_ins->RootOffset + (i - 1) * BYTES_PER_DIR;//根下该目录只有一级
      return 0;
    }
    ...
    {
      return find_subdir(fat16_ins, Root, paths, pathDepth, 1, offset_dir);//根下的子目录中一定存在且其中内容不为空,为多级目录
    }
    ...
    {
      sector_read(fat16_ins->fd, fat16_ins->FirstRootDirSecNum + RootDirCnt, buffer);//读取目录区中下一个扇区(32*16=512)
      RootDirCnt++;
    }
  }
...
int find_subdir(FAT16 *fat16_ins, DIR_ENTRY *Dir, char **paths, int pathDepth, int curDepth, off_t *offset_dir)
{
  ...
  ClusterN = Dir->DIR_FstClusLO;//文件的首簇号

  first_sector_by_cluster(fat16_ins, ClusterN, &FatClusEntryVal, &FirstSectorofCluster, buffer);//找该簇号对应的FAT表项FatClusEntryVal,第一个扇区号FirstSectorofCluster,第一个扇区的内容

  /* Searching for the given path in all directory entries of Dir */
  for (uint i = 1; Dir->DIR_Name[0] != 0x00; i++)//目录簇以扇区中目录条目项为0X00为结束标识,目录应当整数倍的划分扇区空间,这类情况对应的可能是最后一个目录条目没有塞满整个扇区
  {
    memcpy(Dir, &buffer[((i - 1) * BYTES_PER_DIR) % BYTES_PER_SECTOR], BYTES_PER_DIR);//取buffer中一个目录条目大小（32B）的内容到Dir中
    ...
    {
      *offset_dir = get_cluster_offset(fat16_ins, ClusterN) + (DirSecCnt - 1) * BYTES_PER_SECTOR + (i - 1) * BYTES_PER_DIR;//簇偏移+扇区偏移+目录条目偏移
      return 0;
    }
    ...
    if (i % 16 == 0)//如果i % 16 != 0  则 继续读一下个目录条目,否则读下一个扇区、簇
    {
      /* If there are still sector to be read in the cluster, read the next sector. */
      if (DirSecCnt < fat16_ins->Bpb.BPB_SecPerClus)
      {
        sector_read(fat16_ins->fd, FirstSectorofCluster + DirSecCnt, buffer);//当前扇区读完了，读该簇内的下一个扇区
        DirSecCnt++;
      }
      else//如果一个簇内扇区读完了
      { /* Reaches the end of the cluster */
        ...
        if (FatClusEntryVal == 0xffff)//FAT表项读到EOF了，所有簇已读完毕，没有找到
        {
          return 1;
        }

        /* Next cluster */
        ClusterN = FatClusEntryVal;//通过FAT找到下一个簇号

        first_sector_by_cluster(fat16_ins, ClusterN, &FatClusEntryVal, &FirstSectorofCluster, buffer);//重新赋值给FAT表项，簇内第一个扇区号
...
/**
 * @brief 读取path对应的目录名，结果通过filler函数写入buffer中
 * 
 * @param path    要读取目录的路径
 * @param buffer  结果缓冲区
                  ...
                  off_t offset, struct fuse_file_info *fi)
{
  ...
  if (strcmp(path, "/") == 0)
  {
    ...
    for (uint i = 1; i <= fat16_ins->Bpb.BPB_RootEntCnt; i++)
    {
       ...
       **/
      /*** BEGIN ***/
      memcpy(&Root, &sector_buffer[((i - 1) * BYTES_PER_DIR) % BYTES_PER_SECTOR], BYTES_PER_DIR);//取buffer中一个目录条目大小（32B）的内容
      if (Root.DIR_Name[0] == 0x00){
      return 0;
    }
    int is_ok = (Root.DIR_Name[0] != 0xe5)&&(Root.DIR_Name[0] != 0x0f);

    if(is_ok && (Root.DIR_Attr== ATTR_ARCHIVE || Root.DIR_Attr == ATTR_DIRECTORY)){
  ...
  else  // 要查询的目录不是根目录
  {
    ...
    DWORD FirstSectorofCluster;  // 该簇的第一个扇区号
    char* filename;             // 文件名
    ...
    for (uint i = 1; Dir.DIR_Name[0] != 0x00; i++){
      ...
      int is_ok = (Dir.DIR_Name[0] != 0xe5)&&(Dir.DIR_Name[0] != 0x0f);
      
      if(is_ok && (Dir.DIR_Attr== ATTR_ARCHIVE || Dir.DIR_Attr == ATTR_DIRECTORY)){
               ...
               struct fuse_file_info *fi)
{
   ...
   **/
  /*** BEGIN ***/
  DIR_ENTRY Dir;//文件所在的目录项
  off_t offset_dir;//该目录项在镜像文件中的偏移量
  size_t read_size;
  off_t beginBytes = 0,
        endBytes = 0,
        sector_offset = 0;
  ...
  if(offset >= Dir.DIR_FileSize){
    ...
    return -EINVAL;
  }else if(endBytes>=Dir.DIR_FileSize){ 
    read_size = Dir.DIR_FileSize-offset;
    endBytes = Dir.DIR_FileSize-1;
  }else{
    read_size = size;
    endBytes = offset + size - 1;
  }

  WORD beginCluster, ClusterN, FatClusEntryVal;
  DWORD beginSector, FirstSectorofCluster;
  BYTE sector_buffer[BYTES_PER_SECTOR];
  int DirSecCnt = 0;
  //begin所在簇、扇区、扇区中的位置
  ClusterN = Dir.DIR_FstClusLO;

  FatClusEntryVal = fat_entry_by_cluster(fat16_ins, ClusterN);//FAT表项
  beginBytes = 0;
  while( (beginBytes + fat16_ins->ClusterSize) <= offset && ClusterN != CLUSTER_END){//以簇为力度查找
    beginBytes += fat16_ins->ClusterSize;
  ...
  for (size_t i = 0; i < read_size; i++){//往Buffer写
    ...
    buffer[i] = sector_buffer[sector_offset++];
  }  
...
// ------------------TASK2: 创建/删除文件-----------------------------------
/**
 * @brief 在path对应的路径创建新文件,path的最后一层即待创建的文件名
 * 
 * @param path    要创建的文件路径
...
int fat16_mknod(const char *path, mode_t mode, dev_t devNum)
{
  ...
  else
  {
     ...
     **/    
    /*** BEGIN ***/
    if(find_root(fat16_ins, &Dir, prtPath, &offset_dir)==1){
      return -ENOENT;
    } 
    ClusterN = Dir.DIR_FstClusLO;
  ...
  if (findFlag == 1) {
    ...
    /*** BEGIN ***/
    return dir_entry_create(fat16_ins, sectorNum, offset_sector, paths[pathDepth - 1], ATTR_ARCHIVE, 0xffff, 0);
    /*** END ***/
  }else{
    return -ENOENT;
  }
...
int free_cluster(FAT16 *fat16_ins, int ClusterNum)
{
  ...
  WORD fat_offset = ClusterNum * 2;
  //FAT1
  WORD FatSecNum = fat16_ins->Bpb.BPB_RsvdSecCnt + (fat_offset / fat16_ins->Bpb.BPB_BytsPerSec);//因为是按扇区来读Buffer的，所以应当先找到到簇以扇区为单位的偏移量
  WORD FatEntOffset = fat_offset % fat16_ins->Bpb.BPB_BytsPerSec;

  for (uint i = 0; i < fat16_ins->Bpb.BPB_NumFATS; i++) {
    sector_read(fd, FatSecNum, sector_buffer);

    sector_buffer[FatEntOffset] = 0x00;
    sector_buffer[FatEntOffset+1] = 0x00;

    sector_write(fd, FatSecNum, sector_buffer);

    FatSecNum += fat16_ins->Bpb.BPB_FATSz16;//FAT2
  }
...
int fat16_unlink(const char *path)
{
  ...
  DIR_ENTRY Dir,Root;
  off_t offset_dir;
  //释放使用过的簇, find_from_root 查找待删除文件在父目录中目录条目的文件属性
  if (find_root(fat16_ins, &Dir, path, &offset_dir) == 1)//如果文件已被删除，这里find_root返回1
  {
    return -ENOENT;
  }
  ...
  /*** BEGIN ***/
  if(Dir.DIR_Attr == ATTR_DIRECTORY){//删除文件夹,返回错误
    return EISDIR;
  }
  ...
  BYTE sector_buffer[BYTES_PER_SECTOR];
  DWORD sectorNum;
  int offset_sector, offset, i, findFlag = 0, RootDirCnt = 1, DirSecCnt = 1;
  WORD FatClusEntryVal;
  DWORD FirstSectorofCluster;
  ...
  if (strcmp(prtPath, "/") == 0)
  {
    ...
    for ( i = 1; i <= fat16_ins->Bpb.BPB_RootEntCnt; i++){
      ...
      int is_del ;
      is_del = (Root.DIR_Name[0] == 0xe5);
      int is_eq = 0;
      if(!is_del)//重名问题
        is_eq = strncmp((const char *)Root.DIR_Name, paths[0], 11) == 0 ? 1 : 0;
      if(Root.DIR_Name[0] != 0x0f){
  ...
  else /* Else if parent directory is sub-directory */
  {
     ...
     **/
    /*** BEGIN ***/
    char filename[MAX_SHORT_NAME_LEN] ;
    filename[11] = '\0';
    if(find_root(fat16_ins, &Dir, prtPath, &offset_dir)==1){
      printf("no ptrFile\n");
      return -ENOENT;
    ...
    ClusterN = Dir.DIR_FstClusLO;
    first_sector_by_cluster(fat16_ins, ClusterN, &FatClusEntryVal, &FirstSectorofCluster, sector_buffer);//找该簇号对应的FAT表项FatClusEntryVal,第一个扇区号FirstSectorofCluster,第一个扇区的内容
    if(Dir.DIR_Attr == ATTR_ARCHIVE){//父目录不能是文件
      return  ENOTDIR;
    }
    printf("%s(searching for )\n", paths[pathDepth-1]);
    for(i = 1; Dir.DIR_Name[0] != 0x00; i++){//读父目录中的条目
      memcpy(&Dir, &sector_buffer[((i - 1) * BYTES_PER_DIR) % BYTES_PER_SECTOR], BYTES_PER_DIR);//取buffer中一个目录条目大小（32B）的内容到Dir中
      int is_eq = 0 ;
      if(Dir.DIR_Name[0] != 0xe5){
        memcpy(filename, Dir.DIR_Name, 11);
        printf("%s\n", filename);
        is_eq =  strncmp((const char *)Dir.DIR_Name, paths[pathDepth-1], 11) == 0 ? 1 : 0;
      }
      if(Dir.DIR_Name[0] != 0x0f){//不是长文件名
      ...
      if (i % 16 == 0) {
        ...
        if (DirSecCnt < fat16_ins->Bpb.BPB_SecPerClus) {
          ...
          if(FatClusEntryVal == 0xffff){//子目录的簇用完了
            break;
            findFlag=0;
          }
          //读取下一个簇（即簇号为FatClusEntryVal的簇）的第一个扇区
  ...
  if (findFlag == 1){
    ...
    // sector_buffer[offset_sector] = 0xe5;
    // sector_write(fat16_ins, sectorNum, sector_buffer);
    FILE *fd = fat16_ins->fd;
    BYTE DelEnt = 0xe5;
    fseek(fd, sectorNum*fat16_ins->Bpb.BPB_BytsPerSec + offset_sector,SEEK_SET);
    fwrite(&DelEnt, sizeof(BYTE), 1, fd);
    fflush(fd);
    return 0;
    /*** END ***/
  }
